...
@section('page-subtitle', 'Tambahkan program keahlian baru ke sistem.')
...
<form action="{{ route('admin.jurusan.store') }}" method="POST" id="jurusanForm" novalidate>
@csrf
{{-- Jika dipanggil dari halaman lain (mis: tambah siswa) --}}
            ...
            <div class="card-header bg-white border-bottom py-3">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        ...
                        <small class="text-muted">Nama, kode, dan deskripsi program keahlian</small>
                    </div>
                </div>
            </div>
            <div class="card-body">
                <div class="row g-3">
                    ...
                    <div class="col-12">
                        ...
                        <div class="form-text">Nama lengkap program keahlian.</div>
                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="description" id="deskripsiInput"
                                  ...
                                  rows="3"
                                  placeholder="Deskripsi singkat program keahlian ini...">{{ old('description') }}</textarea>
    ...
    <div class="col-lg-5">
            ...
            <div class="position-relative" style="z-index:1;">
                <div class="d-flex align-items-center gap-3 mb-3">
                    ...
                    <div class="overflow-hidden flex-grow-1">
                        <div id="previewNama" class="fw-bold text-white fs-6 text-truncate">Nama Jurusan</div>
                        <div id="previewDesc" class="text-white opacity-75 small text-truncate">—</div>
                    </div>
                </div>
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-white border-bottom py-3">
                <h6 class="mb-0 fw-semibold">
                    <i class="fas fa-info-circle me-2 text-success"></i>Contoh Jurusan Kesehatan
                </h6>
            </div>
            <div class="card-body py-3">
                <ul class="list-unstyled mb-0 small">
                    @foreach([
                        ['Keperawatan', 'KEP'],
                        ['Farmasi', 'FAR'],
                        ['Teknologi Lab. Medik', 'TLM'],
                        ['Asisten Keperawatan', 'AKP'],
                    ] as [$nama, $kode])
                    <li class="d-flex align-items-center justify-content-between mb-2 pb-2 border-bottom">
                        <span class="text-dark">{{ $nama }}</span>
                        <span class="badge bg-success bg-opacity-10 text-success fw-semibold">{{ $kode }}</span>
                    </li>
                    @endforeach
                </ul>
            </div>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-body d-flex flex-column gap-2">
                ...
                <a href="{{ route('admin.jurusan.index') }}" class="btn btn-outline-danger">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    namaEl.addEventListener('blur', function () {
        if (!kodeEl.value && this.value.trim()) {
            const words = this.value.trim().split(/\s+/);
            let kode;
            if (words.length === 1) {
                kode = words[0].substring(0, 4).toUpperCase();
            } else {
                kode = words.map(w => w[0].toUpperCase()).join('').substring(0, 5);
            }
            kodeEl.value = kode;
            updatePreview();
        }
    ...
    window.addEventListener('pageshow', function (e) {
        ...
        const btn = document.getElementById('submitBtn');
        btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-save me-2"></i>Simpan Jurusan';
    });
