    ...
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-white border-bottom d-flex justify-content-between align-items-center py-3">
            <h6 class="mb-0 fw-semibold">
                <i class="fas fa-flask me-2 text-primary"></i>Daftar Praktikum
            </h6>
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0 small">
                    ...
                    <tbody>
                        ...
                        @php $isPublished = $practical->published_at || $practical->is_published; @endphp
                        <tr>
                            <td class="ps-4">
                                <input type="checkbox" class="form-check-input practical-checkbox"
                                       value="{{ $practical->id }}">
                            </td>
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    <div class="rounded-2 bg-warning bg-opacity-10 d-flex align-items-center justify-content-center flex-shrink-0"
                                         style="width:34px;height:34px;">
                                        <i class="fas fa-flask text-warning fa-sm"></i>
                                    </div>
                                    <div>
                                        <div class="fw-semibold text-dark">{{ Str::limit($practical->title, 45) }}</div>
                                        @if($practical->description)
                                            <small class="text-muted">{{ Str::limit($practical->description, 55) }}</small>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            ...
                            <td>
                                @if($practical->due_date)
                                    @php $due = \Carbon\Carbon::parse($practical->due_date); @endphp
                                    <div class="{{ $due->isPast() ? 'text-danger' : 'text-dark' }}">
                                        {{ $due->format('d M Y') }}
                                    </div>
                                    <small class="text-muted">{{ $due->format('H:i') }}</small>
                                @else
                                    <span class="text-muted">—</span>
                                @endif
                            </td>
                            ...
                            <td class="text-center pe-4">
                                <div class="d-flex gap-1 justify-content-center">
                                    ...
                                    <button type="button"
                                            class="btn btn-outline-{{ $isPublished ? 'secondary' : 'success' }} btn-sm"
                                            onclick="togglePublish({{ $practical->id }})"
                                            title="{{ $isPublished ? 'Sembunyikan' : 'Publikasikan' }}">
                                        <i class="fas fa-{{ $isPublished ? 'eye-slash' : 'check' }}"></i>
                                    </button>
            ...
            <div class="modal-content">
                <div class="modal-header border-0 pb-0">
                    <h6 class="modal-title fw-semibold">
                        <i class="fas fa-exclamation-triangle text-danger me-2"></i>Hapus Praktikum
                    </h6>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body py-2 text-muted small">
                    Hapus praktikum ini? Semua penilaian terkait juga akan dihapus. Tindakan tidak dapat dibatalkan.
                </div>
    ...
    <div class="modal fade" id="bulkDeleteModal" tabindex="-1">
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                ...
                <div class="modal-body py-2 text-muted small">
                    Hapus <strong id="selectedCount">0</strong> praktikum? Tidak dapat dibatalkan.
                </div>
                <div class="modal-footer border-0 pt-0">
                    <button type="button" class="btn btn-outline-secondary btn-sm" data-bs-dismiss="modal">Batal</button>
                    <form id="bulkDeleteForm" method="POST"
                          action="{{ route('admin.practicals.bulk-delete') }}" class="d-inline">
                        @csrf
                        <button type="submit" class="btn btn-danger btn-sm">Hapus</button>
                    </form>
                </div>
...
function deletePractical(id) {
    document.getElementById('deleteForm').action =
        '{{ route("admin.practicals.destroy", ":id") }}'.replace(':id', id);
