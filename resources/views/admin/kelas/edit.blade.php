...
<div class="row g-4">
    ...
    <div class="col-lg-8">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-8">
                        ...
                        <select name="major_id" id="majorSelect"
                            ...
                            @foreach($jurusans as $j)
                                <option value="{{ $j->id }}"
                                        data-name="{{ $j->name }}"
                                        {{ (int)$selectedJurusanId === (int)$j->id ? 'selected' : '' }}>
                    ...
                    <div class="col-12">
                        ...
                        <div class="form-text">
                            ...
                            Tahun ajaran akan diisi otomatis dari periode yang dipilih.
                            Saat ini: <strong>{{ $kelas->academic_year ?? '—' }}</strong>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="status" id="statusSelect" class="form-select">
                            <option value="active"   {{ old('status', $kelas->status ?? 'active') == 'active'   ? 'selected' : '' }}>
                                Aktif
                            </option>
                            <option value="inactive" {{ old('status', $kelas->status ?? 'active') == 'inactive' ? 'selected' : '' }}>
                                Nonaktif
                            </option>
                        </select>
                    </div>
        ...
        @include('admin.kelas.partials.subject-picker', [
            ...
            'selectedSubjectIds' => old('subject_ids', $selectedSubjectIds),
            'kelasJurusanId'     => old('major_id', $kelas->jurusan_id),
        ])
            ...
            <div class="position-relative" style="z-index:1;">
                <div class="d-flex align-items-center gap-3 mb-3">
                    ...
                    <div class="overflow-hidden flex-grow-1">
                        ...
                        <div id="previewMajor" class="text-white opacity-75 small">
                            {{ $kelas->jurusan?->name ?? '—' }}
                        </div>
                    </div>
                </div>
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-white border-bottom py-3">
                <h6 class="mb-0 fw-semibold">
                    <i class="fas fa-info-circle me-2 text-primary"></i>Info Kelas
                </h6>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.updatePreview = function () {
        ...
        const periodOpt = document.getElementById('periodSelect')?.options[document.getElementById('periodSelect')?.selectedIndex];
    ...
    document.getElementById('periodSelect')?.addEventListener('change', function () {
        ...
        const semEl  = document.getElementById('semesterSelect');
        if (semEl && semVal) semEl.value = semVal;
        updatePreview();
    });
    ...
    document.getElementById('kelasForm').addEventListener('submit', function (e) {
        ...
        if (!grade) {
            e.preventDefault();
            alert('Tingkat kelas harus dipilih (X, XI, atau XII).');
            return;
        }
